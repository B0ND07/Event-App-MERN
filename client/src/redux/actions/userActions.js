...
export const getEventsAction = (username) => async (dispatch) => {
  try {
    dispatch(setLoading(true));
    const { data } = await axios.get(
      `http://localhost:5006/api/events/user/${username}`
    );
...
export const deleteUserAction = (username) => async (dispatch) => {
  try {
    await axios.delete(`http://localhost:5006/api/users/delete/${username}`);
  } catch (err) {
    dispatch(setError(err.response.data.message));
  }
};
